    ...
    def reconfigure_files(
        ...
        for file in files:
            ...
            if (
                ...
                and files.src_uris.get(
                    file.src_uri.replace(".md", "") + expected_locale_file_ending
                )
                is not None
            ):
                log.debug(
                    f"Skipping '{file.src_uri}' because the version with {expected_locale_file_ending} exists"
                )
                continue
